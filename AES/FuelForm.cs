...
namespace AES
{
    public partial class FuelForm : Form
    {
        ...
        private void LoadData()
        {
            ...
            {
                conn.Open();
                OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT * FROM Топливо", conn);
        ...
        {
            EditFuel editForm = new EditFuel(null);
        ...
        private void DataGridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                string fuelId = dataGridView.Rows[e.RowIndex].Cells["ID_Топливо"].Value.ToString();
                EditFuel editForm = new EditFuel(fuelId);
                if (editForm.ShowDialog() == DialogResult.OK)
                {
                    LoadData();
                }
